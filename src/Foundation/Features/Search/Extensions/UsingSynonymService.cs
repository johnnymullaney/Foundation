...
namespace Foundation.Features.Search.Extensions
{
    ...
    public class UsingSynonymService
    {
        ...
        public IQueriedSearch<TSource, QueryStringQuery> UsingSynonyms<TSource>(IQueriedSearch<TSource> search, TimeSpan? cacheDuration = null)
        {
            ...
            {
                EPiServer.Find.Tracing.Trace.Instance.Add(new TraceEvent(search, "Your index lacks an admin index. Please contact support.") { IsError = false });
            ...
            return new Search<TSource, QueryStringQuery>(search, context =>
            {
                if (context.RequestBody.Query != null)
                {
                    ...
                    if (!QueryHelpers.GetFirstQueryStringQuery(context, out currentMinShouldMatchQueryStringQuery, out currentBoolQuery))
                    {
                        ...
                        EPiServer.Find.Tracing.Trace.Instance.Add(new TraceEvent(search, "The use of synonyms are only supported for QueryStringQueries, i.e. with the use of the .For() -extensions. The query will be executed without the use of synonyms.") { IsError = false });
                        return;
                    }
